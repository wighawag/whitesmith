# Task: Implement "Add --dry-run flag to the run command"

## Context

You are implementing a task generated from GitHub Issue #13: "Add `--dry-run` flag to the `run` command"

**Issue URL:** https://github.com/wighawag/whitesmith/issues/13
**Task ID:** 13-001
**Task File:** tasks/13/001-add-dry-run-flag.md
...
5. **Delete the task file** at `tasks/13/001-add-dry-run-flag.md` — this marks the task as complete.
6. **Clean up**: if the task directory `tasks/13/` is now empty, delete it too.
7. **Commit** all changes (implementation + task file deletion):

```
git add -A
git commit -m "feat(#13): Add --dry-run flag to the run command"
```
...
- You MUST delete `tasks/13/001-add-dry-run-flag.md` as part of your commit.
- If the `tasks/13/` directory is empty after deletion, remove it.
